...
class MMG(torch.nn.Module):
    ...
    def forward(self, obj_feature_3d, obj_feature_2d, edge_feature_3d, edge_feature_2d, edge_index, batch_ids, istrain=False):
        ...
        for i in range(batch_size):
            ...
            center_A = obj_feature_3d[None, idx_i, :3].clone().detach().repeat(len(idx_i), 1, 1)
            center_B = obj_feature_3d[idx_i, None, :3].clone().detach().repeat(1, len(idx_i), 1)
        ...
        #obj_feature_3d = self.self_attn[i](obj_feature_3d, obj_feature_3d, obj_feature_3d, attention_weights=distance, way=attention_matrix_way, attention_mask=mask)
        obj_feature_2d = self.self_attn(obj_feature_2d, obj_feature_2d, obj_feature_2d, attention_weights=distance, way=attention_matrix_way, attention_mask=mask)
        #obj_feature_2d = self.cross_attn[i](obj_feature_3d, obj_feature_2d, obj_feature_2d, attention_weights=distance, way=attention_matrix_way, attention_mask=mask)
        obj_feature_2d = self.cross_attn(obj_feature_2d, obj_feature_3d, obj_feature_3d, attention_weights=distance, way=attention_matrix_way, attention_mask=mask)
        ...
        for i in range(self.depth):
            ...
            if i < (self.depth-1) or self.depth==1:
                ...
                obj_feature_2d = F.relu(obj_feature_3d)
                obj_feature_2d = self.drop_out(obj_feature_3d)
...
class MMG_Debug(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, dim_atten, num_heads=1, aggr= 'max', 
        ...
        # self.concat_2d_obj = nn.Linear(512 * 2, 512)
        # self.concat_2d_rel = nn.Linear(512 * 2, 512)

        # self.self_attn = MultiHeadAttention(d_model=dim_node, d_k=dim_node // num_heads, d_v=dim_node // num_heads, h=num_heads)
        # self.cross_attn = MultiHeadAttention(d_model=dim_node, d_k=dim_node // num_heads, d_v=dim_node // num_heads, h=num_heads)
        ...
        # self.cross_attn_fc = nn.Sequential(  # 11 32 32 4(head)
        #     nn.Linear(11, 32),  # xyz, dist
        #     nn.ReLU(),
        #     nn.LayerNorm(32),
        #     nn.Linear(32, 32),
        #     nn.ReLU(),
    ...
    def forward(self, obj_feature_3d, obj_feature_2d, edge_feature_3d, edge_feature_2d, edge_index, batch_ids, obj_center=None, discriptor=None, istrain=False):
        ...
        # compute weight for rel
        # if discriptor is not None:
            
        #     N_K = edge_feature_3d.shape[0]
        #     # get attention weight for center
        #     center_distance = discriptor[edge_index[0], 0:3] - discriptor[edge_index[1], 0:3]
        #     center_A = center_distance[None, :, :].clone().detach().repeat(N_K, 1, 1)
        #     center_B = center_distance[:, None, :].clone().detach().repeat(1, N_K, 1)
        #     dist_center = center_A - center_B
        #     # get attention weight for center
        #     std_distance = discriptor[edge_index[0], 3:6] - discriptor[edge_index[1], 3:6]
        #     std_A = std_distance[None, :, :].clone().detach().repeat(N_K, 1, 1)
        #     std_B = std_distance[:, None, :].clone().detach().repeat(1, N_K, 1)
        #     dist_std = std_A - std_B
        #     # get attention weight for dim ratio
        #     dim_distance = discriptor[edge_index[0], 6:9] / discriptor[edge_index[1], 6:9]
        #     dim_A = dim_distance[None, :, :].clone().detach().repeat(N_K, 1, 1)
        #     dim_B = dim_distance[:, None, :].clone().detach().repeat(1, N_K, 1)
        #     dist_dim = torch.log(dim_A / dim_B)
        #     # get attention weight for volume
        #     volume_distance = discriptor[edge_index[0], 9:10] / discriptor[edge_index[1], 9:10]
        #     volume_A = volume_distance[None, :, :].clone().detach().repeat(N_K, 1, 1)
        #     volume_B = volume_distance[:, None, :].clone().detach().repeat(1, N_K, 1)
        #     dist_volume = torch.log(volume_A / volume_B)
        #     # get attention weight for ratio
        #     length_distance = discriptor[edge_index[0], 10:11] / discriptor[edge_index[1], 10:11]
        #     length_A = length_distance[None, :, :].clone().detach().repeat(N_K, 1, 1)
        #     length_B = length_distance[:, None, :].clone().detach().repeat(1, N_K, 1)
        #     dist_length = torch.log(length_A / length_B)
            
        #     weights = torch.cat([dist_center, dist_std, dist_dim, dist_volume, dist_length], dim=-1).unsqueeze(0)  # 1 8 N N
            
        #     rel_distance_weight = self.cross_attn_fc(weights).permute(0,3,1,2)  # 1 num_heads N N
        #     rel_attention_matrix_way = 'add'
        # else:
        #     mask = None
        #     rel_distance_weight = None
        #     rel_attention_matrix_way = 'mul'
        ...
        for i in range(self.depth):
            ...
            #obj_feature_2d = self.cross_attn[i](obj_feature_2d, obj_feature_3d, obj_feature_3d, attention_mask=mask)
            
            obj_feature_3d = obj_feature_3d.squeeze(0)
            obj_feature_2d = obj_feature_2d.squeeze(0)  

            #obj_feature_2d = self.concat_2d_obj(torch.cat((obj_feature_2d, obj_feature_3d), dim=-1))

            obj_feature_3d, edge_feature_3d = self.gcn_3ds[i](obj_feature_3d, edge_feature_3d, edge_index, istrain=istrain)
            obj_feature_2d, edge_feature_2d = self.gcn_2ds[i](obj_feature_2d, edge_feature_2d, edge_index, istrain=istrain)

            #edge_feature_2d = self.concat_2d_rel(torch.cat((edge_feature_2d, edge_feature_3d), dim=-1))
...
class MMG_Debug_single(torch.nn.Module):
...
class MMG_Debug2(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, dim_atten, num_heads=1, aggr= 'max', 
        ...
        self.cross_attn_obj = nn.ModuleList(
            MultiHeadAttention(d_model=dim_node, d_k=dim_node // num_heads, d_v=dim_node // num_heads, h=num_heads) for i in range(depth))
    ...
    def forward(self, obj_feature_3d, obj_feature_2d, edge_feature_3d, edge_feature_2d, edge_index, batch_ids, obj_center=None, istrain=False):
        ...
        obj_feature_3d = self.self_attn(obj_feature_3d, obj_feature_3d, obj_feature_3d, attention_weights=distance, way=attention_matrix_way, attention_mask=mask)
        #obj_feature_2d = self.cross_attn(obj_feature_2d, obj_feature_3d, obj_feature_3d, attention_weights=distance, way=attention_matrix_way, attention_mask=mask)
        ...
        for i in range(self.depth):
            ...
            edge_feature_2d = self.cross_attn_rel[i](edge_feature_2d, edge_feature_3d, edge_feature_3d)
            obj_feature_2d = self.cross_attn_obj[i](obj_feature_2d, obj_feature_3d, obj_feature_3d)

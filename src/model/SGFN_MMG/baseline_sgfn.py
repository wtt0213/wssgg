...
from src.utils.eva_utils import get_gt, evaluate_topk_object, evaluate_topk_predicate, evaluate_triplet_topk
from src.utils._eval_utils_2 import evaluate_triplet_recallk, evaluate_triplet_mrecallk
...
class Baseline(BaseModel):
    ...
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        # self.obj_predictor = torch.nn.Linear(512, num_obj_class)
    ...
    def process_val2(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, use_triplet=False):
        ...
        if use_triplet:
            ...
            predcls_gc = evaluate_triplet_recallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=[20, 50, 100], topk_each=1, use_clip=False,  evaluate='rels')
            # np.array([predcls_R20_ngc, predcls_R50_ngc, predcls_R100_ngc])
            predcls_ngc = evaluate_triplet_recallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=[20, 50, 100], topk_each=100, use_clip=False,  evaluate='rels')
            # np.array([sgcls_R20_gc, sgcls_R50_gc, sgcls_R100_gc])
            sgcls_gc = evaluate_triplet_recallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=[20, 50, 100], topk_each=1, use_clip=False,  evaluate='triplet')
            # np.array([sgcls_R20_ngc, sgcls_R50_ngc, sgcls_R100_ngc])
            sgcls_ngc = evaluate_triplet_recallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=[20, 50, 100], topk_each=100, use_clip=False,  evaluate='triplet')
        else:
            # top_k_triplet = [101]
            # cls_matrix = None
            # sub_scores = None
    ...
    def process_val3(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, use_triplet=False):
        ...
        if use_triplet:
            #top_k_triplet, cls_matrix, sub_scores, obj_scores, rel_scores = evaluate_triplet_topk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=101, use_clip=False, obj_topk=top_k_obj)
            # np.array([predcls_R20_gc, predcls_R50_gc, predcls_R100_gc])
            predcls_gc = evaluate_triplet_mrecallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=[20, 50, 100], topk_each=1, use_clip=False,  evaluate='rels')
            # np.array([predcls_R20_ngc, predcls_R50_ngc, predcls_R100_ngc])
            predcls_ngc = evaluate_triplet_mrecallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=[20, 50, 100], topk_each=100, use_clip=False,  evaluate='rels')
            # np.array([sgcls_R20_gc, sgcls_R50_gc, sgcls_R100_gc])
            sgcls_gc = evaluate_triplet_mrecallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=[20, 50, 100], topk_each=1, use_clip=False,  evaluate='triplet')
            # np.array([sgcls_R20_ngc, sgcls_R50_ngc, sgcls_R100_ngc])
            sgcls_ngc = evaluate_triplet_mrecallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=[20, 50, 100], topk_each=100, use_clip=False,  evaluate='triplet')
        else:
            # top_k_triplet = [101]
            # cls_matrix = None
            # sub_scores = None

...
from src.model.model_utils.network_MMG import MMG_Debug, MMG_Debug2
...
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        self.object_projector_3d = torch.nn.Sequential(
            torch.nn.Linear(512, 512),
            torch.nn.LayerNorm(512)
        )

        self.object_projector_2d = torch.nn.Sequential(
            torch.nn.Linear(512, 512),
            torch.nn.LayerNorm(512)
        )
        ...
        self.optimizer = optim.AdamW([
            ...
            {'params':self.object_projector_3d.parameters(), 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            {'params':self.object_projector_2d.parameters(), 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            {'params':self.triplet_projector_3d.parameters(), 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            {'params':self.triplet_projector_2d.parameters(), 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
    ...
    def init_weight(self, obj_label_path, rel_label_path, adapter_path):
        torch.nn.init.xavier_uniform_(self.object_projector_3d[0].weight)
        torch.nn.init.xavier_uniform_(self.object_projector_2d[0].weight)
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        """ Create 3d object feature """
        obj_3d_feats = self.obj_encoder(obj_points)
        ...
        """ Pass attention based gnn """
        obj_center = descriptor[:, :3].clone()
        gcn_obj_feature_3d, gcn_obj_feature_2d, gcn_edge_feature_3d, gcn_edge_feature_2d \
            = self.mmg(obj_3d_feats, obj_2d_feats, rel_feature_3d, rel_feature_2d, edge_indices, batch_ids, obj_center, istrain=istrain)

        """ Compute triplet loss """
        gcn_trip_feature_3d = self.generate_object_pair_features(gcn_obj_feature_3d, gcn_edge_feature_3d, edge_indices)
        gcn_trip_feature_2d = self.generate_object_pair_features(gcn_obj_feature_2d, gcn_edge_feature_2d, edge_indices)
        
        gcn_trip_feature_3d = self.triplet_projector_3d(gcn_trip_feature_3d)
        gcn_trip_feature_2d = self.triplet_projector_2d(gcn_trip_feature_2d)

        """ Save object feature to compute mimic loss """
        obj_feature_3d_mimic = gcn_obj_feature_3d.clone()
        obj_feature_2d_mimic = gcn_obj_feature_2d.clone()

        """ Compute predicate logits """
        rel_cls_3d = self.rel_predictor_3d(gcn_edge_feature_3d)
        rel_cls_2d = self.rel_predictor_2d(gcn_edge_feature_2d)
        
        """ Compute object logits """
        ...
        if istrain:
            return obj_logits_3d, obj_logits_2d, rel_cls_3d, rel_cls_2d, obj_feature_3d_mimic, obj_feature_2d_mimic, gcn_trip_feature_3d, gcn_trip_feature_2d, logit_scale
        else:
    ...
    def process_train(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None):
        ...
        obj_logits_3d, obj_logits_2d, rel_cls_3d, rel_cls_2d, obj_feature_3d, obj_feature_2d, edge_feature_3d, edge_feature_2d, obj_logit_scale = self(obj_points, obj_2d_feats, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=True)
        ...
        loss_mimic = F.l1_loss(obj_feature_3d, obj_feature_2d.detach())
        ...
        rel_mimic_2d = self.cosine_loss(edge_feature_2d, rel_text_feat, t=0.9)
        ...
        loss = lambda_o * (loss_obj_2d + loss_obj_3d) + 3 * lambda_r * (loss_rel_2d + loss_rel_3d) + 0.1 * (rel_mimic_2d + rel_mimic_3d + loss_mimic)

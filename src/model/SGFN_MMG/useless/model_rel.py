...
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        #     torch.nn.Dropout(0.1)
        # )
        # self.mlp_mimic = torch.nn.Sequential(
        ...
        self.optimizer = optim.AdamW([
            ...
            #{'params':self.mlp_mimic.parameters(), 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
    ...
    def init_weight(self, obj_label_path, rel_label_path, adapter_path):
        ...
        #torch.nn.init.xavier_uniform_(self.mlp_mimic[0].weight)
    ...
    def get_rel_emb(self, objs_target, rels_target, edges):
        ...
        for edge_index in range(len(edges)):
            ...
            if rels_target[edge_index].sum() == 0:
                ...
                #target_rel_tokens.append(clip.tokenize(f"the {target_eo} and the {target_os} has no relation"))
                rel_index.append(edge_index)
                non_rel_index.append(edge_index)
            else:
        ...
        # set the non-rel edge to zero
        # target_rel_feats[non_rel_index] = 0
        target_rel_feats = target_rel_feats / target_rel_feats.norm(dim=-1, keepdim=True)
        
        return target_rel_feats.float(), non_rel_index
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        if istrain:
            #obj_feature_3d_mimic = self.mlp_mimic(obj_feature[..., :512].clone())
            #obj_feature_3d_mimic = self.mlp_3d(obj_feature)
        ...
        gcn_edge_feature_3d_dis = gcn_edge_feature_3d.clone()
        gcn_edge_feature_2d_dis = gcn_edge_feature_2d.clone()
        ...
        if istrain:
            return obj_logits_3d, obj_logits_2d, rel_cls_3d, rel_cls_2d, obj_feature_3d_mimic, obj_features_2d_mimic, gcn_edge_feature_3d_dis, gcn_edge_feature_2d_dis, logit_scale
        else:
    ...
    def process_train(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None):
        ...
        rel_text_feat, no_rel_idx = self.get_rel_emb(gt_cls, gt_rel_cls, edge_indices)

        edge_feature_2d = edge_feature_2d / edge_feature_2d.norm(dim=-1, keepdim=True)
        #edge_feature_2d[no_rel_idx] = 0
        ...
        #edge_feature_3d[no_rel_idx] = 0
        rel_mimic_3d = F.l1_loss(edge_feature_3d, rel_text_feat)
               
        loss = lambda_o * (loss_obj_2d + loss_obj_3d) + 3 * lambda_r * (loss_rel_2d + loss_rel_3d) + 0.1 * (rel_mimic_2d + rel_mimic_3d + loss_obj_KL_3d + loss_rel_KL_2d) + 0.1 * loss_mimic

...
from src.model.model_utils.network_MMG import MMG_Debug
from src.model.model_utils.network_GNN import GraphEdgeAttenNetworkLayers
from src.model.model_utils.network_PointNet import PointNetfeat, PointNetCls, PointNetRelCls, PointNetRelClsMulti2, PointNetRelClsMulti
...
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        # self.gcn2d = GraphEdgeAttenNetworkLayers(512,
        #             256,
        ...
        # self.gcn3d = GraphEdgeAttenNetworkLayers(512,
        #             256,
        #             self.mconfig.DIM_ATTEN,
        #             self.mconfig.N_LAYERS, 
        #             self.mconfig.NUM_HEADS,
        ...
        # mmg_obj2d, mmg_rel2d = [], []
        # for name, para in self.gcn2d.named_parameters():
        #     if 'nn_edge' in name:
        #         mmg_rel2d.append(para)
        #     else:
        #         mmg_obj2d.append(para)
        
        # mmg_obj3d, mmg_rel3d = [], []
        # for name, para in self.gcn3d.named_parameters():
        #     if 'nn_edge' in name:
        #         mmg_rel3d.append(para)
        #     else:
        #         mmg_obj3d.append(para)
        ...
        self.optimizer = optim.AdamW([
            ...
            # {'params':mmg_obj2d, 'lr':float(config.LR) / 2, 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            # {'params':mmg_rel2d, 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            # {'params':mmg_obj3d, 'lr':float(config.LR) / 2, 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            # {'params':mmg_rel3d, 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
    ...
    def init_weight(self, obj_label_path, rel_label_path, adapter_path):
        ...
        text_features_obj,  text_features_rel = self.get_label_weight(obj_label_path, rel_label_path)
        # node feature classifier        
        self.obj_predictor_2d = torch.nn.Linear(self.mconfig.clip_feat_dim, self.num_class)
        self.obj_predictor_2d.weight.data.copy_(text_features_obj)
        ...
        self.obj_predictor_3d.weight.data.copy_(text_features_obj)
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        gcn_obj_feature_3d, gcn_obj_feature_2d, gcn_edge_feature_3d, gcn_edge_feature_2d \
            = self.mmg(obj_feature, obj_2d_feats, rel_feature_3d, rel_feature_2d, edge_indices, batch_ids, obj_center, istrain=istrain)
        ...
        if istrain:
            ...
            return obj_logits_3d, obj_logits_2d, rel_cls_3d, rel_cls_2d, obj_feature_3d_mimic, obj_features_2d_mimic, gcn_obj_feature_2d, logit_scale
        else:
            obj_logits_3d = logit_scale * self.obj_predictor_3d(gcn_obj_feature_3d / gcn_obj_feature_3d.norm(dim=-1, keepdim=True))
            obj_logits_2d = logit_scale * self.obj_predictor_2d(gcn_obj_feature_2d / gcn_obj_feature_2d.norm(dim=-1, keepdim=True))
    ...
    def process_train(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None):
        ...
        obj_logits_3d, obj_logits_2d, rel_cls_3d, rel_cls_2d, obj_feature_3d, obj_feature_2d, gcn_obj_feature_2d, logit_scale = self(obj_points, obj_2d_feats, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=True)
        ...
        loss = lambda_o * (loss_obj_2d + loss_obj_3d) + 3 * lambda_r * (loss_rel_2d + loss_rel_3d) + 0.1 * loss_mimic
        # + 0.1 * (loss_rel_KL_2d + loss_obj_KL_3d)
        self.backward(loss)

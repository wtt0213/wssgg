...
from src.model.model_utils.network_MMG import MMG2
...
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        '''
        3d cat location sa, 2d cat location sa, no mimic, KL
        '''
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        obj_3d_feature_mimic = obj_feature[..., :512].clone()
        ...
        if istrain:
            return obj_logits_3d, obj_logits_2d, rel_cls_3d, rel_cls_2d, obj_3d_feature_mimic, obj_2d_feats_mimic
    ...
    def process_train(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None):
        ...
        obj_logits_3d, obj_logits_2d, rel_cls_3d, rel_cls_2d, obj_3d_feature_mimic, obj_2d_feats_mimic = self(obj_points, obj_2d_feats, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=True)
        ...
        loss_obj_KL_3d = F.kl_div(obj_logits_3d_kl.log(), obj_logits_2d_kl, reduction='sum')
        ...
        # compute loss for mimic
        obj_3d_feature_mimic = obj_3d_feature_mimic / torch.norm(obj_3d_feature_mimic, dim=-1, keepdim=True)
        obj_2d_feats_mimic = obj_2d_feats_mimic / torch.norm(obj_2d_feats_mimic, dim=-1, keepdim=True)
        mimic_loss = F.l1_loss(obj_3d_feature_mimic, obj_2d_feats_mimic)
        ...
        #loss = lambda_o * (loss_obj_2d + loss_obj_3d) + 2 * lambda_r * (loss_rel_2d + loss_rel_3d) + 10 * mimic_loss + 0.05 * loss_obj_KL_3d + loss_rel_KL_2d
        loss = lambda_o * (loss_obj_2d + loss_obj_3d) + 2 * lambda_r * (loss_rel_2d + loss_rel_3d) + 0.1 * mimic_loss + 0.05 * loss_obj_KL_3d + loss_rel_KL_2d
        self.backward(loss)
        ...
        log = [("train/rel_loss", loss_rel_3d.detach().item()),
                ...
                ("train/mimic_loss", mimic_loss.detach().item()),
                ("train/loss_obj_KL_3d", loss_obj_KL_3d.detach().item()),

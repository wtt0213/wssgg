...
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        self.mlp_2d = torch.nn.Sequential(
        ...
        self.mlp_fintune_2d = torch.nn.Sequential(
            torch.nn.Linear(512 + 8, 512),
            torch.nn.BatchNorm1d(512),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.1)
        )

        self.mlp_fintune_3d = torch.nn.Sequential(
            torch.nn.Linear(512 + 8, 512),
            torch.nn.BatchNorm1d(512),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.1)
        ...
        self.optimizer = optim.AdamW([
            ...
            {'params':self.mlp_2d.parameters(), 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            {'params':self.mlp_fintune_2d.parameters(), 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            {'params':self.mlp_fintune_3d.parameters(), 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
        ])
    ...
    def init_weight(self, label_path, adapter_path):
        ...
        torch.nn.init.xavier_uniform_(self.mlp_fintune_2d[0].weight)
        torch.nn.init.xavier_uniform_(self.mlp_fintune_3d[0].weight)
        text_features_2d, text_features_3d = self.get_label_weight(label_path)
        # node feature classifier        
        self.obj_predictor_2d = torch.nn.Linear(self.mconfig.clip_feat_dim, self.num_class)
        self.obj_predictor_2d.weight.data.copy_(text_features_2d)
        ...
        self.obj_predictor_3d.weight.data.copy_(text_features_2d)
    ...
    def get_label_weight(self, label_path):
        ...
        text_2d = torch.cat([clip.tokenize(f"a photo of a {c}") for c in label_list]).cuda()
        text_3d = torch.cat([clip.tokenize(f"a point cloud of a {c}") for c in label_list]).cuda()
        with torch.no_grad():
            text_features_2d = model.encode_text(text_2d)
            text_features_3d = model.encode_text(text_3d)
        
        text_features_2d /= text_features_2d.norm(dim=-1, keepdim=True)
        text_features_3d /= text_features_3d.norm(dim=-1, keepdim=True)

        return text_features_2d.float(), text_features_3d.float()
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        obj_2d_feats = torch.cat([obj_2d_feats, obj_feature], dim=-1)
        obj_2d_feats = self.mlp_2d(obj_2d_feats)
        
        if self.mconfig.USE_SPATIAL:
            tmp = descriptor[:,3:].clone()
            tmp[:,6:] = tmp[:,6:].log() # only log on volume and length
            obj_3d_feats = torch.cat([obj_feature, tmp],dim=-1)
            obj_2d_feats = torch.cat([obj_2d_feats, tmp],dim=-1)

        
        gcn_obj_feature_3d, gcn_obj_feature_2d, gcn_edge_feature_3d, gcn_edge_feature_2d \
            = self.mmg(obj_3d_feats, obj_2d_feats, rel_feature_3d, rel_feature_2d, edge_indices, batch_ids, istrain=istrain)
        
        gcn_obj_feature_2d = self.mlp_fintune_2d(gcn_obj_feature_2d)
        gcn_obj_feature_3d = self.mlp_fintune_3d(gcn_obj_feature_3d)
    ...
    def process_train(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None):
        ...
        obj_logits_2d_kl = F.softmax(obj_logits_2d.clone(), dim=-1)
        ...
        if self.mconfig.multi_rel_outputs:
            ...
            rel_logits_3d_kl = F.softmax(rel_cls_3d.clone(), dim=-1)
        ...
        loss = lambda_o * (loss_obj_2d + loss_obj_3d) + 2 * lambda_r * (loss_rel_2d + loss_rel_3d) + 0.1 * loss_mimic + loss_rel_KL_2d + 0.1 * loss_obj_KL_3d
        #loss = lambda_o * (loss_obj_2d + loss_obj_3d) + 2 * lambda_r * (loss_rel_2d + loss_rel_3d) + 10 * loss_mimic
        self.backward(loss)

...
class MMGNet_SGPN():
    ...
    @torch.no_grad()
    def data_processing_train(self, items):
        obj_points, gt_class, rel_points, gt_rel_cls, edge_indices = items 
        obj_points = obj_points.permute(0,2,1).contiguous()
        rel_points = rel_points.permute(0,2,1).contiguous()
        obj_points, gt_class, rel_points, gt_rel_cls, edge_indices = \
            self.cuda(obj_points, gt_class, rel_points, gt_rel_cls, edge_indices)
        return obj_points, gt_class, rel_points, gt_rel_cls, edge_indices
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in loader:
                ...
                ''' get data '''
                obj_points, gt_class, rel_points, gt_rel_cls, edge_indices = self.data_processing_train(items)
                logs = self.model.process_train(obj_points, gt_class, rel_points, gt_rel_cls, edge_indices)
    ...
    def validation(self, debug_mode = False):
        ...
        for i, items in enumerate(val_loader, 0):
            ...
            ''' get data '''
            obj_points, gt_class, rel_points, gt_rel_cls, edge_indices = self.data_processing_val(items)            
            ...
            with torch.no_grad():
                ...
                # else:
                # top_k_obj, top_k_obj_2d, top_k_rel, top_k_rel_2d, tok_k_triplet, top_k_2d_triplet, cls_matrix, sub_scores, obj_scores, rel_scores \
                #     = self.model.process_val(obj_points, gt_class, rel_points, gt_rel_cls, edge_indices, use_triplet=True)
                top_k_obj, top_k_obj_2d, top_k_rel, top_k_rel_2d, predcls_gc, predcls_ngc, sgcls_gc, sgcls_ngc \
                    =  self.model.process_val(obj_points, gt_class, rel_points, gt_rel_cls, edge_indices, use_triplet=True)
        ...
        # mean_recall = get_mean_recall(topk_triplet_list, cls_matrix_list)
        # mean_recall_2d = get_mean_recall(topk_triplet_2d_list, cls_matrix_list)
        # zero_shot_recall = get_zero_shot_recall(topk_triplet_list, cls_matrix_list, self.dataset_valid.classNames, self.dataset_valid.relationNames)
        # zero_shot_recall_2d = get_zero_shot_recall(topk_triplet_2d_list, cls_matrix_list, self.dataset_valid.classNames, self.dataset_valid.relationNames)
        ...
        if self.model.config.EVAL:
            ...
            # np.save(os.path.join(save_path,'obj_scores_list.npy'), obj_scores_list)
            # np.save(os.path.join(save_path,'rel_scores_list.npy'), rel_scores_list)
            f_in = open(os.path.join(save_path, 'result_2.txt'), 'w')
        # else:
        #     f_in = None   
        ...
        # print(f"Eval: 3d zero-shot recall@50 : {zero_shot_recall[0]}", file=f_in)
        # print(f"Eval: 3d zero-shot recall@100: {zero_shot_recall[1]}", file=f_in)
        # print(f"Eval: 2d zero-shot recall@50 : {zero_shot_recall_2d[0]}", file=f_in)
        # print(f"Eval: 2d zero-shot recall@100: {zero_shot_recall_2d[1]}", file=f_in)
        print(f"Eval :3d sgcls gc recall@20 :{recall_sgcls_gc[0]}")
        print(f"Eval :3d sgcls gc recall@50 :{recall_sgcls_gc[1]}")
        print(f"Eval :3d sgcls gc recall@100 :{recall_sgcls_gc[2]}")
        print(f"Eval :3d sgcls ngc recall@20 :{recall_sgcls_ngc[0]}")
        ...
        logs = [("Acc@1/obj_cls_acc", obj_acc_1),
                ...
                #("zero_shot_recall@50", zero_shot_recall[0]),
                #("zero_shot_recall@100", zero_shot_recall[1]),
                #("zero_shot_2d_recall@50", zero_shot_recall_2d[0]),
                #("zero_shot_2d_recall@100", zero_shot_recall_2d[1]),
                ]

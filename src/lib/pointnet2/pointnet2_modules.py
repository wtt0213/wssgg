...
class PointnetSAModuleVotes(nn.Module):
    ...
    def __init__(
        ...
        mlp_spec = mlp
        #self.mlp_module_ini = pt_utils.SharedMLP(mlp, bn=bn)
    ...
    def forward(self, xyz: torch.Tensor,
                ...
                initial_features: torch.Tensor = None,
                use_ini=False) -> (torch.Tensor, torch.Tensor):
        r"""
        Parameters
        ----------
        xyz : torch.Tensor
        ...
        if not self.ret_unique_cnt:
            ...
            if use_ini:
                grouped_initial_features, grouped_initial_xyz = self.grouper(
                    xyz, new_xyz, initial_features
                )  # (B, C, npoint, nsample)
                grouped_initial_features = grouped_initial_features[:,3:,:,:]
                #grouped_initial_features[:, 0:3, :, :] = 0
                grouped_initial_features = torch.mean(grouped_initial_features, dim=3)
        else:
        ...
        """
        if use_ini:
            new_features_ini = self.mlp_module(
                grouped_initial_features
            )  # (B, mlp[-1], npoint, nsample)
            new_features_ini = F.max_pool2d(
                new_features_ini, kernel_size=[1, new_features_ini.size(3)]
            )  # (B, mlp[-1], npoint, 1)
            grouped_initial_features = new_features_ini.squeeze(-1)  # (B, mlp[-1], npoint)
        """
        if not self.ret_unique_cnt:
            if use_ini:
                return new_xyz, new_features, inds, grouped_initial_features
            else:
                return new_xyz, new_features, inds
        else:
            return new_xyz, new_features, inds, unique_cnt

class PointnetSAModuleVotes2Data(nn.Module):
    ''' Modified based on _PointnetSAModuleBase and PointnetSAModuleMSG
    with extra support for returning point indices for getting their GT votes '''
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        if not self.ret_unique_cnt:
            ...
            grouped_features2, grouped_xyz2, unique_cnt2 = self.grouper(
                xyz, new_xyz, features2
            )  # (B, C, npoint, nsample), (B,3,npoint,nsample), (B,npoint)
        ...
        if self.pooling == 'max':
            ...
            new_features2 = F.max_pool2d(
                new_features2, kernel_size=[1, new_features2.size(3)]
            )  # (B, mlp[-1], npoint, 1)
        elif self.pooling == 'avg':
            new_features = F.avg_pool2d(
                new_features, kernel_size=[1, new_features.size(3)]
        ...
        if not self.ret_unique_cnt:
            if use_ini:
                return new_xyz, new_features, new_features2, inds, grouped_initial_features
            else:
                return new_xyz, new_features, new_features2, inds
        else:
            return new_xyz, new_features, new_features2, inds, unique_cnt

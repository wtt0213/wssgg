...
if __name__ == '__main__':
    fpath = '/data/caidaigang/project/3DSSG_Repo/test'
    edge_indices = torch.load(fpath + '/edge_indices.pth')
    gt_edges = torch.load(fpath + '/gt_edges.pth')
    obj_logits = torch.load(fpath + '/obj_logits.pth')
    rel_cls = torch.load(fpath + '/rel_cls.pth')
    # print(len(gt_edges))
    # print(obj_logits.shape, rel_cls.shape)
    conf = evaluate_triplet_recallk(objs_pred=obj_logits, rels_pred=rel_cls, gt_rel=gt_edges, edges=edge_indices, multi_rel_outputs=True,
                                    topk=[10, 20, 50, 100], topk_each=100, use_clip=True, evaluate='triplet')
    # conf = evaluate_triplet_recallk(objs_pred=obj_logits, rels_pred=rel_cls, gt_rel=gt_edges, edges=edge_indices, multi_rel_outputs=True,
    #                                topk=[ 100], topk_each=100, use_clip=True, evaluate='triplet')
    print(conf)
